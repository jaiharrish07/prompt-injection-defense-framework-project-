"""
Demonstration script for the OpenAI integration in PromptGuard
"""
import os
from mitigation_engine import MitigationEngine

def demo_without_openai():
    """Demonstrate PromptGuard without OpenAI (traditional analysis)"""
    print("=" * 60)
    print("PROMPTGUARD TRADITIONAL ANALYSIS (Without OpenAI)")
    print("=" * 60)
...
def demo_openai_behavior_simulation():
    """Simulate how OpenAI would behave with and without PromptGuard"""
    print("\n" + "=" * 60)
    ...
    for i, item in enumerate(dangerous_prompts, 1):
        ...
        # Analyze with PromptGuard
        result = engine.analyze_prompt(prompt)
        
        print(f"   PromptGuard Action: {result['action']}")
        print(f"   Risk Score: {result['risk_score']}/100")
        
        # Simulate what would happen to OpenAI
        if result['action'] == 'BLOCK':
            print(f"   OpenAI Response: BLOCKED BY PROMPTGUARD - Malicious content detected")
    ...
    print("=" * 60)
    print("✓ PromptGuard acts as security middleware")
...
def show_api_endpoints():
    ...
    print("\nNEW API ENDPOINTS:")
    print("GET  / .................. Main UI")
    print("POST /analyze ......... Traditional PromptGuard analysis")
